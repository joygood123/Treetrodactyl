    ...
    if (pm2Available) {
      ...
      try {
        ...
        if (newPorts.length > 0) {
          ...
          // Update the port registry to the ACTUAL port the app is using
          // This is written to ports.json and server.js reads it for proxying
          const portsFile = path.join(require('path').dirname(projectRoot), '..', 'ports.json');
          // Update via the global portRegistry path used by server.js
          try {
            const siteRoot  = path.dirname(projectRoot); // /var/www/user-sites/subdomain
            const sitesRoot = path.dirname(siteRoot);    // /var/www/user-sites
          ...
          log(`\x1b[31m[start] ✗ App did not open any port after 4 seconds.\x1b[0m`);
          log(`\x1b[33m[start] Check pm2 logs: docker exec deployboard pm2 logs ${appName}\x1b[0m`);
        }
