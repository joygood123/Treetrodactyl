...
// Write a per-subdomain Nginx config — Docker-aware.
// In Docker Compose, proxy to "app" service hostname; bare metal uses 127.0.0.1.
async function writeNginxProxyConfig(subdomain, port) {
  const inDocker  = fs.existsSync('/.dockerenv');
  const proxyHost = inDocker ? 'app' : '127.0.0.1';
  const configDir = inDocker
    ? path.join(SITES_DIR, '_nginx-apps')
    : (NGINX_SITES_DIR || '/etc/nginx/conf.d/deployboard-apps');
...
`;
  ...
  // Reload Nginx — docker exec first (Docker Compose), bare metal fallback
  try {
    const { execSync } = require('child_process');
    let reloaded = false;

    // Method 1: docker exec into nginx container (Docker Compose setup)
    try {
      execSync('docker exec deployboard-nginx nginx -s reload', { stdio: 'pipe' });
      console.log(`[Nginx] Reloaded via docker exec`);
      reloaded = true;
    } catch(dockerErr) {
      // Method 2: nginx directly on host (bare metal)
      for (const bin of ['/usr/sbin/nginx', '/usr/local/sbin/nginx', '/sbin/nginx']) {
        try {
