...
async function runStartAndCleanup({ project, projectRoot, buildDir, emit, log, onLog, envObj }) {
  ...
  if (startCmd) {
    ...
    if (pm2Available) {
      ...
      try {
        // Start using ecosystem file — this is what guarantees PORT is passed
        await exec('pm2', ['start', ecoPath],
          { cwd: projectRoot, env: { ...process.env, PATH: localBin + ':' + process.env.PATH } },
          log);

        // Save pm2 process list so it can be checked later
        await new Promise(r => {
          const p = spawn('pm2', ['save'], { stdio: 'ignore' });
          p.on('close', r); p.on('error', r);
        });

        // Wait 3 seconds for the app to boot and bind to the port
        await new Promise(r => setTimeout(r, 3000));

        // Verify the app actually started on the right port
        const portOpen = await checkPortOpen('127.0.0.1', parseInt(assignedPort), 3000);
        if (portOpen) {
          log(`\x1b[32m[start] ✓ App is running and listening on port ${assignedPort}\x1b[0m`);
        } else {
          log(`\x1b[33m[start] ⚠ App started via pm2 but port ${assignedPort} is not yet open.\x1b[0m`);
          log(`\x1b[33m[start]   If this app hardcodes a port (like 3000), set "Custom App Port" to that port.\x1b[0m`);
          log(`\x1b[33m[start]   Or make the app read process.env.PORT.\x1b[0m`);
        }

      } catch(e) {
        log(`\x1b[31m[start] pm2 failed to start app: ${e.message}\x1b[0m`);
        log(`\x1b[33m[start] Check your Start Command is correct.\x1b[0m`);
      }

    } else {
      // No pm2 — spawn detached background process
      ...
      try {
        ...
        child.unref();
        log(`\x1b[32m[start] ✓ Started (PID ${child.pid}) on PORT=${assignedPort}\x1b[0m`);
        await new Promise(r => setTimeout(r, 3000));
        const portOpen = await checkPortOpen('127.0.0.1', parseInt(assignedPort), 3000);
        if (!portOpen) {
          log(`\x1b[33m[start] ⚠ Port ${assignedPort} not open. App may have crashed or uses a hardcoded port.\x1b[0m`);
        }
      } catch(e) {
...
/** Check if a TCP port is open (app is listening) within timeoutMs */
